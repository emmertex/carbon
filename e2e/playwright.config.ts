...
// Choose owned disposable state before Playwright starts webServer. An explicit
// E2E_DATA_DIR is a parent directory, never a database to erase or reuse.
const parent = process.env.E2E_DATA_DIR ?? tmpdir();
mkdirSync(parent, { recursive: true });
const dataDir =
  process.env.E2E_RUN_DATA_DIR ?? mkdtempSync(join(parent, "carbon-e2e-"));
process.env.E2E_RUN_DATA_DIR = dataDir;
...
const E2E_AUTH_USERS =
  "alice:114e2e4c08550bb4210c675e6e83da71ce3e4f30f352ed981d878a1fd9dbea45";

/**
 * Functional E2E: real web UI + live sync server. One worker, fresh server DB per
 * run (config evaluation), fresh client storage per test (helpers/reset.ts).
 */

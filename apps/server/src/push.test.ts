...
describe("POST /push/subscribe", () => {
  ...
  test("returns 400 when subscription is missing keys", async () => {
    ...
    const res = await appFetch(app, "/push/subscribe", {
      method: "POST",
      headers: { Authorization: basic, "Content-Type": "application/json" },
      body: JSON.stringify({ endpoint: "https://8.8.8.8/x" }),
    });
...
describe("POST /push/unsubscribe", () => {
  test("removes an existing subscription", async () => {
    ...
    const app = buildPushApp(db, vapidPublicKey);
    const endpoint = "https://8.8.8.8/todelete";
    ...
    assert.equal(res.status, 200);
    assert.ok(
      !db.get("SELECT 1 FROM push_subscriptions WHERE endpoint = ?", [
        endpoint,
      ]),
    );
  });
...
describe("checkReminders", () => {
  test("skips due tasks that carry an on-hold tag", async () => {
    ...
    const sent = (id: string) =>
      !!db.get(
        "SELECT 1 AS x FROM reminders_sent WHERE item_id = ? AND kind = ?",
        [id, "due"],
      );
...
describe("notifyTask WebPush error handling", () => {
  test("a non-404/410 send error is logged and the subscription is kept", async () => {
    const { db, deviceId, addUser } = makeTestDb();
    const { id: userId } = addUser("u", "pw");
    const endpoint = "https://8.8.8.8/server-error";
    ...
    assert.ok(
      logged.some((a) =>
        a.some((x) => String(x).includes("web push send failed")),
      ),
  ...
  test("a private/LAN push endpoint is refused by the SSRF guard before any socket opens (A2)", async () => {
    const { db, deviceId, addUser } = makeTestDb();
    const { id: userId } = addUser("u-ssrf", "pw");
    // A client-supplied endpoint pointing at the cloud-metadata service: a malicious
    // client registering this would have the server POST to it and reflect the creds.
    const malicious =
    ...
    assert.equal(sendCalls, 0, "no socket may open to a private push endpoint");
    assert.ok(
      db.get("SELECT 1 FROM push_subscriptions WHERE endpoint = ?", [
        malicious,
      ]),
      "the subscription is kept (we skip, we do not silently delete it)",
    );
  });
  ...
  test("a 410 (gone) error is not logged as an unexpected failure and cleans up the subscription", async () => {
    ...
    const endpoint = "https://8.8.8.8/gone";
    saveSubscription(db, userId, {
      endpoint,
      keys: { p256dh: "A", auth: "B" },
    });
...
describe("per-tenant VAPID", () => {
  test("each tenant signs with its own keys, not a process-global pair", async () => {
    ...
    webpush.sendNotification = (async (
      ...
      signedWith[sub.endpoint] = (
        opts as { vapidDetails?: { publicKey: string } }
    ...
    assert.equal(signedWith["https://8.8.8.8/tenant-a"], a.vapidPublicKey);
    assert.equal(signedWith["https://8.8.8.8/tenant-b"], b.vapidPublicKey);
  });
});
...
describe("notifyTask shared-user fan-out", () => {
  test("notifies users the task is shared with, including inherited shares", async () => {
    ...
    try {
      const sent = await notifyTask(db, child.id, {
        title: "hi",
        body: "there",
      });
    ...
    assert.deepEqual(endpoints.sort(), [
      "https://8.8.8.8/direct",
      "https://8.8.8.8/inherited",
      "https://8.8.8.8/owner",
    ]);
  });
});
...
describe("POST /push/fcm", () => {
  test("saves an FCM token", async () => {
    ...
    const row = db.get<{ user_id: string }>(
      "SELECT user_id FROM fcm_tokens WHERE token = ?",
      ["fcm-abc-123"],
    );
...
describe("POST /push/fcm/unsubscribe", () => {
  test("removes an FCM token", async () => {
    ...
    assert.ok(
      db.get("SELECT 1 FROM fcm_tokens WHERE token = ?", ["fcm-del-456"]),
    );

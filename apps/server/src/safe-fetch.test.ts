...
describe("assertSafeEndpoint", () => {
  test("rejects malformed URLs and non-http(s) protocols", async () => {
    await assert.rejects(
      () => assertSafeEndpoint("not a url", false),
      EndpointError,
    );
    await assert.rejects(
      () => assertSafeEndpoint("ftp://example.com/", false),
      EndpointError,
    );
  });
  test("rejects localhost and private/loopback IP literals", async () => {
    await assert.rejects(
      () => assertSafeEndpoint("http://localhost/", false),
      EndpointError,
    );
    await assert.rejects(
      () => assertSafeEndpoint("http://foo.localhost/", false),
      EndpointError,
    );
    await assert.rejects(
      () => assertSafeEndpoint("http://127.0.0.1/", false),
      EndpointError,
    );
    await assert.rejects(
      () => assertSafeEndpoint("http://169.254.169.254/", false),
      EndpointError,
    );
    await assert.rejects(
      () => assertSafeEndpoint("http://[::1]/", false),
      EndpointError,
    );
    await assert.rejects(
      () => assertSafeEndpoint("http://[::ffff:7f00:1]/", false),
      EndpointError,
    );
    await assert.rejects(
      () =>
  ...
  test("allows public IP literals", async () => {
    await assert.doesNotReject(() =>
      assertSafeEndpoint("http://8.8.8.8/", false),
    );
  });
  test("allowPrivate bypasses the guard entirely", async () => {
    await assert.doesNotReject(() =>
      assertSafeEndpoint("http://127.0.0.1/", true),
    );
    await assert.doesNotReject(() =>
      assertSafeEndpoint("http://localhost/", true),
    );
  });
});
...
describe("safeFetch redirect handling", () => {
  test("a redirect from a public target to cloud metadata is blocked", async () => {
    ...
    await assert.rejects(
      () => safeFetch("http://8.8.8.8/start", false),
      EndpointError,
    );
  });
  ...
  test("gives up after too many redirect hops", async () => {
    globalThis.fetch = (async (url: string | URL) => {
      const key = String(url);
      const next =
        key === "http://8.8.8.8/a" ? "http://1.1.1.1/b" : "http://8.8.8.8/a";
      return new Response("", { status: 302, headers: { location: next } });
    }) as typeof fetch;
    await assert.rejects(
      () => safeFetch("http://8.8.8.8/a", false),
      EndpointError,
    );
  });
  test("redirects into private space are blocked for a POST webhook call too", async () => {
    ...
    await assert.rejects(
      () =>
        safeFetch("http://8.8.8.8/webhook", false, {
          method: "POST",
          body: "{}",
        }),
      EndpointError,
...
describe("safeFetch credential headers across redirects", () => {
  ...
  test("a cross-origin redirect drops the credentials but keeps ordinary headers", async () => {
    ...
    await safeFetch("http://8.8.8.8/llm", false, {
      method: "GET",
      headers: secrets,
    });
    ...
    for (const name of [
      ...
      assert.equal(
        hops[1].headers[name],
        undefined,
        `${name} must not reach the new origin`,
      );
    }
  ...
  test("a same-origin redirect keeps them (the common /path -> /path/ case)", async () => {
    ...
    await safeFetch("http://8.8.8.8/dav", false, {
...
describe("safeFetch response-size bound (A2)", () => {
  test("a response declaring a body over the cap is refused before it is read", async () => {
    // Declare a 10GB body; safeFetch must throw on the Content-Length check, not buffer it.
    ...
    await assert.rejects(
      () => safeFetch("http://8.8.8.8/big", false),
      EndpointError,
    );
  });

  test("a modestly-sized response passes the cap", async () => {
    ...
    const res = await safeFetch("http://8.8.8.8/ok", false);
    assert.equal(res.status, 200);
    assert.equal(await res.text(), body);
  });
});

test("chunked response is byte bounded and cancelled; unconsumed response expires even with caller signal", async () => {
  let cancelled = false;
  globalThis.fetch = (async () =>
    new Response(
      new ReadableStream({
  ...
  const res = await safeFetch("http://8.8.8.8/stream", false);
  await assert.rejects(() => res.arrayBuffer(), /exceeds/);
  assert.ok(cancelled);
  cancelled = false;
  ...
  assert.ok(
    cancelled,
    "deadline cancels unread body even though caller signal never aborted",
  );
});

test("native push response chunks cannot grow beyond the shared byte limit", async () => {
  const { EventEmitter } = await import("node:events");
  const { boundNativeResponse } = await import("./safe-fetch");
  ...
  boundNativeResponse(request as never);
  request.emit("response", response);
  for (let i = 0; i < 17; i++) response.emit("data", Buffer.alloc(1024 * 1024));
  assert.match(failure?.message ?? "", /byte limit/);
});

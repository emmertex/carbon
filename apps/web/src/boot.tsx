...
import { startSyncLoop, syncNow, restoreSessionOrGate, fetchHostInfo } from './lib/sync';
...
function bootApp(): void {
  ...
  const e2eActive =
    import.meta.env.DEV &&
    (import.meta.env.VITE_CARBON_E2E === '1' || localStorage.getItem('carbon.e2e') === '1');
  ...
  async function boot(): Promise<void> {
    try {
      ...
      await fetchHostInfo();
      // Restore a saved session (or present the sign-in gate) for this workspace,
      // honouring a per-workspace sign-out-by-choice — see restoreSessionOrGate.
      void restoreSessionOrGate();
      startSyncLoop();
      void syncNow();

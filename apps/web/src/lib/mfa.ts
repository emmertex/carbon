...
export async function loginWithPassword(
  ...
  try {
    ...
    if (JSON.stringify(cfg) !== JSON.stringify(getServerConfig()))
      return { status: "error" };
...
export async function mfaEnrollEmailStart(
  challenge: string,
  email: string,
): Promise<void> {
  const cfg = getServerConfig();
...
export async function mfaEnrollEmailConfirm(
  challenge: string,
  code: string,
): Promise<void> {
  const cfg = getServerConfig();
...
export async function mfaEnrollTotpStart(
  ...
  if (!res.ok)
    throw new Error(await errMsg(res, "could not start authenticator"));
  return (await res.json()) as { secret: string; uri: string };
}

export async function mfaEnrollTotpConfirm(
  challenge: string,
  code: string,
): Promise<void> {
  const cfg = getServerConfig();
...
export async function mfaEnrollFinish(
  ...
  if (!res.ok)
    throw new Error(await errMsg(res, "could not finish enrollment"));
  const data = (await res.json()) as {
    token: string;
    recovery_codes?: string[];
    device_token?: string;
...
export async function revokeTrustedDevice(deviceId: string): Promise<void> {
  const res = await fetch(
    joinUrl(
      getServerConfig().url,
      `/api/mfa/devices/${encodeURIComponent(deviceId)}`,
    ),
...
export async function regenerateRecoveryCodes(): Promise<string[]> {
  const res = await fetch(
    joinUrl(getServerConfig().url, "/api/mfa/recovery-codes"),
    {
      method: "POST",
      headers: authHeaders(getServerConfig()),
    },
...
export async function settingsEmailStart(email: string): Promise<string> {
  const res = await fetch(
    joinUrl(getServerConfig().url, "/api/mfa/settings/email/start"),
    {
      method: "POST",
      headers: authHeaders(getServerConfig()),
      body: JSON.stringify({ email }),
...
export async function settingsEmailConfirm(
  challenge: string,
  code: string,
): Promise<void> {
  const res = await fetch(
    joinUrl(getServerConfig().url, "/api/mfa/settings/email/confirm"),
    {
      method: "POST",
      headers: authHeaders(getServerConfig()),
...
export async function settingsTotpStart(): Promise<{
  secret: string;
  uri: string;
}> {
  const res = await fetch(
    joinUrl(getServerConfig().url, "/api/mfa/settings/totp/start"),
    {
      method: "POST",
      headers: authHeaders(getServerConfig()),
    },
...
export async function settingsTotpConfirm(code: string): Promise<void> {
  const res = await fetch(
    joinUrl(getServerConfig().url, "/api/mfa/settings/totp/confirm"),
    {
      method: "POST",
      headers: authHeaders(getServerConfig()),
      body: JSON.stringify({ code }),

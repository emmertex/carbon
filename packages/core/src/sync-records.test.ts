...
test("an unshare after observing the share wins, even against a fast-clock peer", () => {
  ...
  const tombstone = getUnsyncedRecordOps(dbB).find(
    (o) => o.entity === "share",
  )!;
  ingestRecordOps(dbA, [tombstone], true);
  assert.equal(
    listSharesForItem(dbA, "itemX").length,
    0,
    "revoke wins on A too",
  );
});
...
test("a stale timelog edit arriving after a newer one does not clobber it", () => {
  ...
  const createOp = getUnsyncedRecordOps(dbA).find(
    (o) => o.entity === "timelog",
  )!;
  ...
  const noteOp = getUnsyncedRecordOps(dbA).find(
    (o) =>
      o.entity === "timelog" &&
      (o.data as { note: string | null }).note === "stale note",
  )!;
  ...
  assert.equal(
    finalB?.note,
    null,
    "the stale note edit itself is dropped, not applied",
  );
});
...
test("a deleted attachment is not resurrected by replaying its stale create", () => {
  ...
  const row = db.get<{ deleted: number }>(
...
test('itemsMissingCreate is not fooled by "type": appearing inside a field value', () => {
  ...
  const op: Op = {
    ...
    fields: {
      parent_id: "p",
      note: 'mentions "type": here but is not a create',
    },
  };
  ingestOps(db, [op], true); // creates a shell row, no real create op
  assert.deepEqual(
    itemsMissingCreate(db, ["shell1"]),
    ["shell1"],
    "flagged as missing its create",
  );
...
test("ingestRecordOps skips a poison op and still applies the rest of the batch", () => {
  ...
  let skipped: string[] = [];
  assert.doesNotThrow(() => {
    const r = ingestRecordOps(db, [poison, good], true);
    fresh = r.fresh;
    skipped = r.skipped;
  });
  ...
  // A4: the poison op is REPORTED and leaves no log row (compensating delete).
  assert.deepEqual(skipped, ["rop-poison-1"]);
  assert.ok(
    !db.get("SELECT 1 AS x FROM record_ops WHERE id = ?", ["rop-poison-1"]),
    "poison record-op leaves no log row",
  );
  // Re-syncing the same batch stays quiet: the poison op is skipped again (not stored),
...
test("ingestOps tolerates object field values and a missing fields key", () => {
  ...
  const objectFields: Op = {
    ...
    device_id: B,
    fields: {
      recurrence: { type: "weekly", interval: 1 } as unknown as string,
    },
  };
  const noFields = {
    id: "op-nofields-1",
...
test("record ingest rolls back partial statement effects and observes only accepted timestamps", () => {
  const db = openMemoryDb();
  // SQLite FAIL preserves earlier trigger effects unless the ingest rolls back a savepoint.
  db.exec(`CREATE TABLE ingest_audit (id TEXT);
    CREATE TRIGGER fail_share BEFORE INSERT ON shares WHEN NEW.id = 'bad'
    BEGIN INSERT INTO ingest_audit VALUES (NEW.id); SELECT RAISE(FAIL, 'forced failure'); END;`);
  const good = {
    id: "record-good",
    entity: "share",
    row_id: "good",
  ...
  const bad = {
    ...
    row_id: "bad",
    ts: 8e15,
    data: { ...good.data, id: "bad", user_id: "other-user" },
  };
  const result = ingestRecordOps(db, [good, bad, { ...good, ts: 9e15 }], true);
  assert.deepEqual(result.skipped, [bad.id]);
  assert.deepEqual(db.all("SELECT * FROM ingest_audit"), []);
  assert.equal(
    db.get<{ value: string }>("SELECT value FROM meta WHERE key = 'op_clock'")
      ?.value,
    "100",
  );
  assert.equal(db.all("SELECT * FROM record_ops").length, 1);
  db.exec("DROP TRIGGER fail_share");
  assert.equal(
    ingestRecordOps(db, [{ ...bad, ts: 200 }], true).fresh.length,
    1,
  );
});

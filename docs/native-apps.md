...
> Desktop builds currently run the normal web build (`npm run -w @carbon/web build` from
> `tauri.conf.json`) and do **not** force `CARBON_NO_PWA=1` automatically. You can still set
> `CARBON_NO_PWA=1` manually when you need a no-service-worker bundle (e.g. CI/perf runs). The
> AppImage target additionally needs network + FUSE to fetch linuxdeploy on first run.
...
- **System tray** — a tray icon offers _Open Carbon_, _Quick Add_, and _Quit_; a left-click
  opens the quick-add bar directly.
- **Close-to-tray** — closing the main window hides it to the tray (so it keeps receiving
  forwarded quick-add tasks) rather than quitting; only the tray's _Quit_ exits.
...
2. **Server service account** — Firebase console → Project settings → Service accounts
   → _Generate new private key_. Point the Carbon server at it:
   ```
